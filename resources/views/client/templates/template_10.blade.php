...
<style>
    :root {
        --bg-peach: #fff5f2;
        --accent-pink: #ff758f;
        --accent-soft-pink: #ffb3c1;
        --text-dark: #4a3e3d;
    }

    body, html {
        margin: 0; padding: 0; width: 100%; min-height: 100vh;
        background: linear-gradient(180deg, #ffece8 0%, #fff5f2 50%, #ffffff 100%);
    ...
    .countdown { display: flex; justify-content: center; gap: 12px; margin: 25px 0; }
    .countdown .item {
        width: 70px; height: 75px;
        background: #ffffff;
        border-radius: 20px;
        display: flex; flex-direction: column; justify-content: center; align-items: center;
        box-shadow: 0 8px 20px rgba(255, 117, 143, 0.12);
    }
    .countdown .item span { font-size: 24px; font-weight: 700; color: var(--accent-pink); }
    .countdown .item small { font-size: 10px; color: #888; text-transform: uppercase; }

    .gallery-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; margin-top: 15px; }
    .gallery-grid img { width: 100%; height: 180px; object-fit: cover; border-radius: 24px; box-shadow: 0 8px 20px rgba(0,0,0,0.05); cursor: pointer; }
    ...
    .btn-peach:hover { transform: translateY(-2px); color: #fff; }
     /* ================================
...
<div class="card-container">
    ...
    <div class="section-block">
        <span class="text-uppercase fw-bold small text-muted" style="letter-spacing: 3px;">Save Our Date</span>
        
        <div class="my-3">
            <h1 class="font-script title-name" data-field="groom_name">{{ $card->groom_name ?? 'Minh Nhật' }}</h1>
            <div class="fs-4 text-muted my-1">&</div>
            <h1 class="font-script title-name" data-field="bride_name">{{ $card->bride_name ?? 'Tuyết Anh' }}</h1>
        </div>
    ...
    {{-- HAI HỌ --}}
    <div class="section-block">
        <h6 class="fw-bold text-uppercase text-danger-emphasis mb-4" style="letter-spacing: 2px;">THÔNG TIN HAI HỌ</h6>
        <div class="row g-4 text-start">
            <div class="col-6 border-end pe-3">
                <div class="text-center mb-3">
                    <span class="badge bg-danger-subtle text-danger fw-bold px-3 py-1 rounded-pill">NHÀ TRAI</span>
                </div>
            ...
            <div class="col-6 ps-3">
                <div class="text-center mb-3">
                    <span class="badge bg-danger-subtle text-danger fw-bold px-3 py-1 rounded-pill">NHÀ GÁI</span>
                </div>
    ...
    <div class="section-block">
        <div class="row align-items-center g-3 mb-4">
            ...
            <div class="col-7 text-start">
                <span class="badge bg-danger-subtle text-danger mb-1 px-3 py-1 rounded-pill">Chú rể</span>
                ...
                <a id="groom_phone_link" href="tel:{{ $card->groom_phone ?? '0901234567' }}" class="btn btn-sm btn-outline-danger rounded-pill py-0 px-3 small">
        ...
        <div class="row align-items-center g-3">
            <div class="col-7 text-end">
                <span class="badge bg-danger-subtle text-danger mb-1 px-3 py-1 rounded-pill">Cô dâu</span>
                ...
                <a id="bride_phone_link" href="tel:{{ $card->bride_phone ?? '0909876543' }}" class="btn btn-sm btn-outline-danger rounded-pill py-0 px-3 small">
    ...
    {{-- THỜI GIAN & ĐỊA ĐIỂM --}}
    <div class="section-block">
        <h6 class="fw-bold text-uppercase text-danger-emphasis mb-2" style="letter-spacing: 2px;">THỜI GIAN & ĐỊA ĐIỂM</h6>
        <h2 class="display-6 fw-bold my-2" style="color: var(--accent-pink);" data-field="wedding_date">{{ $card->wedding_date ? \Carbon\Carbon::parse($card->wedding_date)->format('d / m / Y') : '28 / 11 / 2026' }}</h2>
       ...
       style="background-color: var(--dark-pink, #ff4d6d); border: none; font-size: 0.82rem;">
        ...
        <div class="pt-3 border-top">
            ...
            <div class="d-flex justify-content-around text-center small">
                <div><strong class="d-block fs-6" style="color: var(--accent-pink);" data-field="time_welcome">{{ $card->time_welcome ?? '11:00' }}</strong><span class="text-muted">Đón khách</span></div>
                <div class="border-end pe-3"></div>
                <div><strong class="d-block fs-6" style="color: var(--accent-pink);" data-field="time_ceremony">{{ $card->time_ceremony ?? '11:30' }}</strong><span class="text-muted">Làm lễ</span></div>
                <div class="border-end pe-3"></div>
                <div><strong class="d-block fs-6" style="color: var(--accent-pink);" data-field="time_party">{{ $card->time_party ?? '12:00' }}</strong><span class="text-muted">Khai tiệc</span></div>
            </div>
        </div>
    </div>
...
<div class="white-card">
    ...
    {{-- GUESTBOOK --}}
    <div class="section-block">
        <h6 class="fw-bold text-uppercase text-danger-emphasis mb-3" style="letter-spacing: 2px;">SỔ LƯU BÚT</h6>
    ...
    {{-- MỪNG CƯỚI & RSVP --}}
    <div class="section-block">
        <h6 class="fw-bold text-uppercase text-danger-emphasis mb-3" style="letter-spacing: 2px;">GỬI NGỌT NGÀO</h6>
        
        <div class="stk-item text-start">
            <div class="d-flex justify-content-between align-items-center mb-1">
                <span class="fw-bold text-uppercase small text-muted">Chú rể</span>
                <span class="badge bg-danger-subtle text-danger" data-field="groom_bank_name">{{ $card->groom_bank_name ?? 'Vietcombank' }}</span>
            </div>
        ...
        <div class="stk-item text-start">
            <div class="d-flex justify-content-between align-items-center mb-1">
                ...
                <span class="badge bg-danger-subtle text-danger" data-field="bride_bank_name">{{ $card->bride_bank_name ?? 'Techcombank' }}</span>
            </div>
            <div class="d-flex justify-content-between align-items-center">
        ...
        <button class="btn btn-peach w-100 py-3 fw-bold mt-4" data-bs-toggle="modal" data-bs-target="#rsvpModal">
            <i class="bi bi-envelope-check me-2"></i>XÁC NHẬN THAM DỰ
        </button>

        <p class="font-script fs-3 text-muted mt-4 mb-0" data-field="thank_msg">"{{ $card->thank_msg ?? 'Sự hiện diện của bạn là niềm hạnh phúc lớn nhất của chúng mình!' }}"</p>
    </div>
